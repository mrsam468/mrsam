...
#         git command-that-updates-a-ref
#     )
set_reflog_action() { 	if [ -z "${GIT_REFLOG_ACTION:+set}" ]; 	then 		GIT_REFLOG_ACTION="$*"; 		export GIT_REFLOG_ACTION; 	fi; }
git_editor() { 	if test -z "${GIT_EDITOR:+set}"; 	then 		GIT_EDITOR="$(git var GIT_EDITOR)" || return $?; 	fi;  	eval "$GIT_EDITOR" '"$@"'; }
git_pager() { 	if test -t 1; 	then 		GIT_PAGER=$(git var GIT_PAGER); 	else 		GIT_PAGER=cat; 	fi; 	for vardef in LESS=FRX LV=-c; 	do 		var=${vardef%%=*}; 		eval ": \"\${$vardef}\" && export $var"; 	done;  	eval "$GIT_PAGER" '"$@"'; }
is_bare_repository () { 	git rev-parse --is-bare-repository; }
cd_to_toplevel () { 	cdup=$(git rev-parse --show-toplevel) && 	cd "$cdup" || { 		gettextln "Cannot chdir to \$cdup, the toplevel of the working tree" >&2; 		exit 1; 	}; }
require_work_tree_exists () { 	if test "z$(git rev-parse --is-bare-repository)" != zfalse; 	then 		program_name=$0; 		die "$(eval_gettext "fatal: \$program_name cannot be used without a working tree.")"; 	fi; }
require_work_tree () { 	test "$(git rev-parse --is-inside-work-tree 2>/dev/null)" = true || { 		program_name=$0; 		die "$(eval_gettext "fatal: \$program_name cannot be used without a working tree.")"; 	}; }
...
# committer.
pick_ident_script () { 	while test $# -gt 0
	do 		lid=$1; shift; 		uid=$1; shift; 		printf '%s' "
		/^$lid /{
			s/'/'\\\\''/g
			h
			s/^$lid "'\([^<]*\) <[^>]*> .*$/\1/'"
...
# feeding to eval.
parse_ident_from_commit () { 	LANG=C LC_ALL=C sed -ne "$(pick_ident_script "$@")"; }
# Parse the author from a commit given as an argument. Stdout is suitable for
# feeding to eval to set the usual GIT_* ident variables.
get_author_ident_from_commit () { 	encoding=$(git config i18n.commitencoding || echo UTF-8); 	git show -s --pretty=raw --encoding="$encoding" "$1" -- | 	parse_ident_from_commit author AUTHOR; }
# Generate a virtual base file for a two-file merge. Uses git apply to
# remove lines from $1 that are not in $2, leaving only common lines.
create_virtual_base() { 	sz0=$(wc -c <"$1"); 	diff -u -La/"$1" -Lb/"$1" "$1" "$2" | git apply --no-add; 	sz1=$(wc -c <"$1"); 
	expr $sz0 \< $sz1 \* 2 >/dev/null || : >"$1"; }
# Platform specific tweaks to work around some commands
case $(uname -s) in *MINGW*) 	if test -x /usr/bin/sort; 	then
		sort () { 			/usr/bin/sort "$@"; 		}; 	fi; 	if test -x /usr/bin/find; 	then
		find () { 			/usr/bin/find "$@"; 		}; 	fi
	if type builtin >/dev/null 2>&1; 	then 		pwd () { 			builtin pwd -W; 		}; 	fi; 	is_absolute_path () { 		case "$1" in 		[/\\]* | [A-Za-z]:*) 			return 0 ;; 		esac; 		return 1; 	}; 	;; *); 	is_absolute_path () { 		case "$1" in 		/*) 			return 0 ;; 		esac; 		return 1; 	}; esac
...
IFS=' 	
'
git_broken_path_fix () { 	case ":$PATH:" in 	*:$1:*) : ok ;; 	*) 		PATH=$(
			SANE_TOOL_PATH="$1"
			IFS=: path= sep=
			set x $PATH
			shift
...
if test -n "$OPTIONS_SPEC"; then 	usage() { 		"$0" -h; 		exit 1; 	};  	parseopt_extra=; 	[ -n "$OPTIONS_KEEPDASHDASH" ] && 		parseopt_extra="--keep-dashdash"; 	[ -n "$OPTIONS_STUCKLONG" ] && 		parseopt_extra="$parseopt_extra --stuck-long";  	eval "$(
		echo "$OPTIONS_SPEC" |
			git rev-parse --parseopt $parseopt_extra -- "$@" ||
		echo exit $?
	)"; else 	dashless=$(basename -- "$0" | sed -e 's/-/ /'); 	usage() { 		die "$(eval_gettext "usage: \$dashless \$USAGE")"; 	};  	if [ -z "$LONG_USAGE" ]; 	then 		LONG_USAGE="$(eval_gettext "usage: \$dashless \$USAGE")"; 	else 		LONG_USAGE="$(eval_gettext "usage: \$dashless \$USAGE

$LONG_USAGE")"; 	fi;  	case "$1" in 		-h) 		echo "$LONG_USAGE"; 		exit; 	esac; fi
...
git --config email ibenjaminsamuel47@gmail.com
cd ~/desktop/git-practice/
git init
dir ~/desktop/git-practice/
cd ~/desktop/git-practice/
cd ~/desktop/task/
git init
open .git
git config --global user.name "samuel"
git config --global user.email ibenjaminsamuel47@gmail.com
git config --global user.name"samuel benjamin"
git config --global user.name "samuel benjamin"
git config --global user.email "ibenjaminsamuel47@gmail.com"
# This shell scriplet is meant to be included by other shell scripts
# to set up some variables pointing at the normal git directories and
...
git config --global user.name "samuel"
git config --global user.email ibenjaminsamuel47@gmail.com
git condig --global core code
git config --global core . code
git config --global core "code"
open
open git
git init
